...
- マルチプラットホーム: Windows，macOS，Linux（Motif，gtk）など
...
Windows，macOSでのバイナリパッケージのインストールは基本的にアーカイブを解凍
...
と打ち込んで下さい．WindowsとmacOSでは，実行ファイルをダブルクリックして
...
開きますので，そこから"指定範囲を再生"を選択して下さい．なお，macOSでは
...
なお，macOSではコマンドラインオプションはサポートされていません．
...
- 読み込み & 書き込み
  ...
  * AIFFファイル（8bit PCM，16bit PCM，24bit PCM，32bit PCM，32bit float PCM）
  * AIFCファイル（8bit PCM，16bit PCM，24bit PCM，32bit PCM，ulaw，alaw）
  * CAFファイル（8bit PCM，16bit PCM，24bit PCM，32bit PCM，32bit float PCM，64bit double PCM，ulaw，alaw）
  * MP3ファイル（MPEG1 layer 3，書き込みには別途プラグインのダウンロードが必要です）
  * Ogg Vorbisファイル
  * FLACファイル
  * ALAC(Apple Lossless Audio Codec)ファイル（拡張子.cafと.mp4に対応）
...
Windows Vista/7/8/10，macOS 10.7-10.14，Linux（CentOS, Ubuntu等）などでの
動作が確認されています．
...
バグ報告の際は，できるだけ以下の項目は記述して下さい．
 ...
 - OSのバージョン（Windows 10 Pro 1809，macOS 10.14.1など）
...
オフィシャルサイトは，
  ...
  http://www-ie.meijo-u.ac.jp/labs/rj001/spLibs/spwave/index-j.html
...
開発し，公開されている方に感謝致します．
  ...
  * ALAC (バージョン0.8.4-3から)
    - the Apple Lossless Audio Codec Project (http://alac.macosforge.org/trac/wiki)
